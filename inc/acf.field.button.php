...
class gp_acf_button extends acf_field
{
    ...
    */
    function render_field( $field ) {
        ...
        <div class="acf-field acf-field-text" style="width:100%;" data-width="100">
            <div class="acf-label">
                <label for="">Style</label>
            </div>
            <div class="acf-input">
                <select name="<?=$field['name']?>[style]" id="<?=$field['id']?>-style" class="<?=$field['style']?>">
                    <option value=""> </option>
                    <option value="is-style-fill" <?=("is-style-fill" == $field['value']['style'] ? 'selected' : '')?>>Fill</option>
                    <option value="is-style-outline" <?=("is-style-outline" == $field['value']['style'] ? 'selected' : '')?>>Outline</option>
                </select>
            </div>
        </div>
        ...
        <div class="acf-field acf-field-text" style="width:33%;" data-width="33">
            ...
            <div class="acf-input">
                <select name="<?=$field['name']?>[color]" id="<?=$field['id']?>-color" class="<?=$field['class']?>">
                    ...
                    <?php // Loop through each master palette
                    foreach ($colors->palettes as $palettes) {

                        // Loop through each sub-palette
                        foreach ($palettes as $palette) { ?>
                            <option value="<?=$palette['slug']?>" <?=($palette['slug'] == $field['value']['color'] ? 'selected' : '')?>><?=$palette['name']?></option>
                        <?php }
