...
function twenty_twenty_one_add_sub_menu_toggle( $output, $item, $depth, $args ) {
	if ( 0 === $depth && in_array( 'menu-item-has-children', $item->classes, true ) ) {
		...
		$output .= '<span class="angle">&angrt;</span>';
...
function twenty_twenty_one_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	...
	$swap_for_social_icon= strpos($args->theme_location, 'footer') !== false || strpos($args->theme_location, 'social') !== false;
	if ('footer' === $args->theme_location || $swap_for_social_icon ) {
