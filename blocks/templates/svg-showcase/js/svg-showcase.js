...
const pop_gdw = function() { // DONE
	if(document.hidden) return;
	svg_target_class = window['pop_gdw_svg_target_class'];
	...
	window['pop_gdw_args_callback'] = function() {
		svg_add_path_classnames(window['pop_gdw_args']);
	}
	...
	args = {
		...
		classname,
		remove_only: false,
		cleanup_callback: window['pop_gdw_args_callback'],
		interval: 50,
	}
...
const pop_gdw2 = function() { // DONE
	if(document.hidden) return;
	svg_target_class = window['pop_gdw2_svg_target_class'];
	...
	window['pop_gdw2_args_callback'] = function() {
		svg_add_path_classnames(window['pop_gdw2_args']);
	}
	...
	args = {
		...
		classname,
		remove_only: false,
		cleanup_callback: window['pop_gdw2_args_callback'],
		interval: 300,
	}
...
const pop_gbdc = function() { // DONE
	if(document.hidden) return;
	svg_target_class = window['pop_gbdc_svg_target_class'];
	...
	window['pop_gbdc_args_callback'] = function() {
		svg_add_path_classnames(window['pop_gbdc_args']);
	}

	args = {svg_target_class, path_target_classes, classname, remove_only: false, cleanup_callback: window['pop_gbdc_args_callback']}
	svg_add_path_classnames( args );
